...
UnpinBuffer(BufferDesc *buf, bool fixOwner)
{
	...
	if (ref->refcount == 0)
	{
		uint32		buf_state;
		uint32		old_buf_state;
	//	bool 		immed_free_buffer = false; //(anfal)
		...
		if (buf_state & BM_PIN_COUNT_WAITER)
		{
			...
			//(anfal) to remove dead pages, we need to check refcount. usagecount is not need since it is LRU implementation
			//if(buf->refcount == 0) //(anfal)
			//	immed_free_buffer = true; //(anfal)
		...
		StrategyFreeBuffer(buf); //(anfal) put a page back to freelist when page is unpinned & reaches a refcount of zero
	}
}
