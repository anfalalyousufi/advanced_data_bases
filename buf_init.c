 ...
 *-------------------------------------------------------------------------
 */
/* Pay attention to the BufferDescriptorsdata structure. 
 * This is the run-time data structure to store information of all 
 * buffer pages I mentioned in class. You have to understand how it is initialized in buf_init.cand 
 * used to maintain the buffer-related information. Depending on how you design the queue, 
 * you may or may not need to modify its definition (in buf_internals.h) and initialization.
 */
...
//#define LRU_NOT_IN_QUEUE (-2) //(anfal)
...
InitBufferPool(void)
{
	...
	else
	{
		...
		for (i = 0; i < NBuffers; i++)
		{
			...
			//buf -> usage_count = 0; //(anfal) not required
			//buf -> refcount = 0;    //(anfal)
			//buf -> flags = 0;		  //(anfal)
			...
			//buf -> lruNext = LRU_NOT_IN_QUEUE; //(anfal)
			//buf -> lruPrev = LRU_NOT_IN_QUEUE; //(anfal)
			/*
